...
void movement_tripod(void)
{
  ...
  float alpha;
	...
	{	
		...
		z[1]=-22+H_onestep-H_onestep*(sqrt(L_onestep*L_onestep+W_onestep*W_onestep)*4*s/T*3/4- sqrt(L_onestep*L_onestep+W_onestep*W_onestep)/8*3)*
		...
		if(s>=T/4&&s<T/2)
		{
			...
			z[3]=-22+H_onestep-H_onestep*(sqrt(L_onestep*L_onestep+W_onestep*W_onestep)*4*(s-T/4)/T*3/4-sqrt(L_onestep*L_onestep+W_onestep*W_onestep)/8*3)*
		...
		if(s>=T/2&&s<T*3/4)
		{
			...
			z[0]=-22+H_onestep-H_onestep*(sqrt(L_onestep*L_onestep+W_onestep*W_onestep)*4*(s-T/2)/T*3/4-sqrt(L_onestep*L_onestep+W_onestep*W_onestep)/8*3)*
		...
		if(s>=3*T/4&&s<T)
		{
			...
			z[2]=-22+H_onestep-H_onestep*(sqrt(L_onestep*L_onestep+W_onestep*W_onestep)*4*(s-3*T/4)/T*3/4-sqrt(L_onestep*L_onestep+W_onestep*W_onestep)/8*3)*
...
void StartAngleInit(void)//darius's robot
{
			/**直角初始位置**/
